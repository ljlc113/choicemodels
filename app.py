...
if page == "Prospect Theory (PT)":
    ...
    st.markdown(" For gains (when x is positive) it follows a compressed graph similar to that of expected utility (EU). This is due to people being risk averse for gains; we prefer to take confirmed gain than gamble. For losses (when x is negative) it follows a convex asymptote shape which grows more quickly than for gains. The steepness is because of loss aversion; we 'value' a loss more than the equivalent reframed gain (e.g. 100% lose 10 vs. 20% lose 40). Also, people are risk seeking for losses; prefer to gamble rather than take a confirmed loss. There are different shapes for the domain of losses and gains because prospect theory takes into account that human judgements differ when we frame things as 'wins' vs 'losses' - framing effect.") 
